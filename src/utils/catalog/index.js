export default function(opts) {
  ...
  // const $scroll_wrap = document.getElementById(Opt.scrollWrapper) || $content.parentNode || document.body   // 内容元素的父元素
  const $scroll_wrap = window    // 监听页面滚动
  ...
  try {
    $catalog.innerHTML = `<div class='cl-wrapper'>${generateHtmlTree(tree, { id: -1 })}<svg class="cl-marker" width="200" height="200" xmlns="http://www.w3.org/2000/svg">
            <path stroke="#ccc" stroke-width="2" fill="transparent" stroke-dasharray="0, 0, 0, 1000" stroke-linecap="round" stroke-linejoin="round" transform="translate(-0.5, -0.5)" />
            </svg></div>`
  } catch (e) {
  ...
  function coolScrollHandler() {
    ...
    const wrapHeight = $scroll_wrap.pageYOffset
  ...
  function simpleScrollHandler(el) {
    ...
    for (let i = allCatalogs.length - 1; i >= 0; i--) {
      if (allCatalogs[i].offsetTop <= $scroll_wrap.pageYOffset) {
