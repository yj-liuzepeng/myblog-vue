...
const coolClockState = reactive({
  ...
  drawCoolClock: () => {
    ...
    let [x0, y0] = [coolClockState.canvas.width / 2, coolClockState.canvas.height / 2] //获取圆心x,y
    ...
    coolClockState.drawTimeNeedle(ctx, x0, y0, 3, minMaxL - 30, oMinValue, '#95cde9') //绘制分针  
    coolClockState.drawTimeNeedle(ctx, x0, y0, 1, minMaxL - 20, oSenValue, '#f3a829') //绘制秒针
    coolClockState.drawSenDotted(ctx, '#60997f', x0, y0, oSenValue - 360, centerL) //绘制秒针尾部
    coolClockState.drawCircleCenter(ctx, x0, y0, '#60997f', centerL) //绘制圆心
    coolClockState.drawRing(ctx, props.ringcolor,coolClockState.canvas.width,coolClockState.canvas.height,100,90) //绘制圆心
  },
  // 绘制表盘外环
...
onMounted(() => {
  // console.log(authorstyle.namecolor)
