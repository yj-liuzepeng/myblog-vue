  ...
  <div>
      ...
      <div class="list_box" v-if="allMusicState.showAllBox">
        ...
        <div class="music_list">
          <div class="list_l">
            <ul class="music_type">
              <li
                v-for="(item, index) in allMusicState.musicTypeList"
                :key="index"
                @click="getMusicType(item.id)"
            ...
            <div class="list_title">
              ...
              <img
                :src="allMusicState.musicStateButton"
                alt=""
                class="music_state"
              ...
              <div class="music_search_box">
                <input
                  type="text"
                  class="music_search"
                  v-model="allMusicState.musicSearchVal"
                  placeholder="搜索歌曲"
                />
                <transition name="music_search">
                  <ul
                    class="search_list"
                    v-if="allMusicState.musicSearchVal !== ''"
                  >
                    <li
                      v-for="(item, index) in allMusicState.musicSearchList"
                      :key="index"
                      @click="ListAdd(item)"
                    >
            ...
            <ul class="list">
              <li
                ...
                :key="index"
                @mouseover="allMusicState.buttonActive(index)"
                @dblclick="
                  musicPlay((allMusicState.thisListPage - 1) * 10 + index)
                "
              >
                <div
                  class="this_music_shlter"
                  v-if="
                    (allMusicState.thisListPage - 1) * 10 + index ===
                    thisMusicIndex
                  "
                ></div>
                <span>{{ item.name }}</span
                ...
                <transition name="list_button">
                  <div
                    class="music_button"
                    v-if="allMusicState.listButtonActiveIndex === index"
                  >
                    <div
                      class="list_play"
                      title="播放这首歌"
            ...
            <div class="list_page">
              <div
                class="page_last"
                v-if="allMusicState.thisListPage !== 1"
              ...
              <div
                ...
                v-if="
                  ...
                  Math.ceil(allMusicState.musicList.length / 10)
                "
          ...
          <div class="list_r">
            ...
            <ul class="music_talk_list">
              <li
                v-for="(item, index) in allMusicState.hotTalkList"
                :key="index"
              >
                <div class="talk_head">
                  <img
    ...
    <div class="bbox" :class="{ bbox_active: curPlayMusic.showBox }">
      <div
        ...
        :class="{ pan_active: curPlayMusic.showBox }"
      ...
      <div
        class="box"
        :style="{ backgroundImage: 'url(' + curPlayMusic.musicImg + ')' }"
        :class="{ box_active: curPlayMusic.showBox }"
        @dblclick="allMusicState.hShowAllBox"
      >
        <div
          class="music_shlter_2"
          :style="{ backgroundImage: 'url(' + curPlayMusic.musicImg + ')' }"
          :class="{ shlter_active: curPlayMusic.showBox }"
        ></div>
        <div
          class="music_shlter"
          :style="{ backgroundImage: 'url(' + curPlayMusic.musicImg + ')' }"
        ...
        <div class="music_dis">
          ...
          <ul class="music_words">
            <div class="music_words_box" :style="{ top: wordsTop + 'px' }">
              <li
                v-for="(item, index) in curPlayMusic.musicWords"
                :key="index"
                class="music_word"
        ...
        <div class="control_box">
          ...
          <div class="progress">
            <div
              class="progress_c"
              :style="{ width: curPlayMusic.currentProgress }"
            >
      ...
      <video
        id="music"
        :autoplay="curPlayMusic.autoplay"
        :src="curPlayMusic.musicUrl"
        name="media"
      ></video>
    </div>
  </div>
...
let curPlayMusic = reactive({
  ...
  musicUrl: "",
  musicImg:
    "https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fimg.zcool.cn%2Fcommunity%2F0137935dd7b203a8012053c08a1cbf.gif&refer=http%3A%2F%2Fimg.zcool.cn&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=auto?sec=1654270133&t=ef1550f78b339d25b3f46b0716a5163c",
...
let allMusicState = reactive({
  ...
  MusicStateChange: () => {
    // 更改模式
    allMusicState.musicState === 0
      ? ((allMusicState.musicState = 1),
...
// 获取歌曲信息
const getInfo = () => {
  getMusicUrl({ id: allMusicState.musicList[thisMusicIndex.value].id }).then(
    (res: any) => {
      let data = JSON.parse(res.data);
      if (
        ...
        if (allMusicState.notPlay.length !== allMusicState.musicList.length) {
          ...
          MusicAlert(
            `${
              allMusicState.musicList[thisMusicIndex.value].name
            }因为一些原因不能播放`
          );
        ...
        getWords({ id: allMusicState.musicList[thisMusicIndex.value].id }).then(
          (res: any) => {
            let data = JSON.parse(res.data);
            if (!data.nolyric) {
        ...
        getHotTalk({
          id: allMusicState.musicList[thisMusicIndex.value].id,
        }).then((res: any) => {
          let data = JSON.parse(res.data);
          let count = 0;
...
const Player = () => {
  ...
  function timer() {
    curPlayMusic.currentProgress = `${
      (player.currentTime / player.duration) * 100
    }%`;
    // 接着这里写歌词滚动
    if (player.currentTime >= wordsTime.value[o.value + 1]) {
      top.value += Number.parseInt(
        $(".music_word").eq(o.value).height() +
          Number.parseInt($(".music_word").eq(o.value).css("marginTop"))
      );
...
const thisMusicList = computed(() => {
  return [...allMusicState.musicList].splice(
    (allMusicState.thisListPage - 1) * 10,
    10
  ); //分页
});
// 监听搜索
watch(
  ...
  () => {
    if (allMusicState.musicSearchVal === "") {
      ...
      getSearchSuggest({ key: allMusicState.musicSearchVal }).then(
        (res: any) => {
          let data = JSON.parse(res.data);
