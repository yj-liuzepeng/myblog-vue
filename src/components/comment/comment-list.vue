  ...
  <div class="comment-list">
    <div class="comment-item" v-for="item in data">
      ...
      <div class="item-content">
        <div class="top">
          ...
          <span v-if="item.from_id == '10'" style="margin-right: 10px">
            <el-tag size="small" type="danger">博主</el-tag></span
          >
          <span class="commenttime">{{
            UnixToDate(new Date(item.create_time), 3)
          }}</span>
          <span class="area">
        ...
        <div class="bottom">
          <span class="like">
            <span
              v-if="userCommentLike.indexOf(String(item.id)) != -1"
              class="icon iconfont icon-dianzan_kuai"
              style="color: #8c8c8c"
              @click="addOrDelLike(item)"
            />
            <span
              v-else
          ...
          <span
            ...
            v-if="item.from_id == userInfoData?.id"
          >
            删除
        ...
        <div class="reply-ipt" v-if="showReply && curReplyId == item.id">
          ...
          <div class="reply-ipt-btns">
            ...
            <el-button type="primary" size="small" @click="sendComment(item)"
              >发送</el-button
            >
          </div>
        </div>
          ...
          <div class="children" v-for="child in item.children">
            ...
            <div class="children-content">
              <div class="children-top">
                ...
                <span v-if="child.from_id == '10'" style="margin-right: 10px">
                  <el-tag size="small" type="danger">博主</el-tag></span
                >
                <span class="commenttime">{{
                  UnixToDate(new Date(child.create_time), 3)
                }}</span>
                <span class="area">
                  <!-- <span class="iconfont icon-didian"></span> -->
                  {{ child.from_area }}
              ...
              <div class="children-bottom">
                <span class="like">
                  <span
                    v-if="userCommentLike.indexOf(String(child.id)) != -1"
                    class="icon iconfont icon-dianzan_kuai"
                    style="color: #8c8c8c"
                    @click="addOrDelLike(child)"
                  />
                  <span
                    v-else
                    class="icon iconfont icon-dianzan"
                ...
                <span
                  ...
                  v-if="child.from_id == userInfoData?.id"
                >
                  删除
                  <!-- <span class="icon iconfont icon-shanchu_o"></span> -->
                </span>
    ...
    <div class="more">
      ...
      <span v-else class="no-more"
        >已经到底了～～<span class="get-more" @click="getLess">收起</span></span
      >
    </div>
  </div>
</template>
    ...
    await addorDelLikeNum(params).then((res: any) => {
      if (res.code == 200) {
        rfhUserCommentLike({
          ...
          like: userInfoData.value.comment_like,
        }).then((r) => {
          if (r.code == 200) {
            emit("refresh");
          }
...
watch(
  ...
  (newval) => {
    ...
    userCommentLike.value = userInfoData.value?.comment_like
      ? userInfoData.value.comment_like.split(",")
      : [];
  }
);
onMounted(() => {
